  ...
  $.fn.extend({
    chromaHash: function(options) {
      ...
      var incrementingID = 0,
          colors = ["primary", "secondary", "tertiary", "quaternary"].slice(0, options.bars);
      ...
      var trigger = function(e) {
        ...
        if ($input.val() === "") {
          $colorBars.css({opacity: 0});
          return;

        }

        // fade the bars in
        $colorBars.css({opacity: 1});
      ...
      */
      ...
      // CLEAN UP CHROMA HASH
      if(options === 'destroy') {

        // unbind all events and clear memory
        this.each(function() {
          var $input = $(this);
          $input.unbind('keyup', $input.data('chroma-hash-trigger')).unbind('blur', $input.data('chroma-hash-trigger'));
          $input.data('chroma-hash-colorBars').remove();
          $input.removeData('chroma-hash-colorBars');
          $input.removeData('chroma-hash-trigger');
        });
      ...
      this.each(function() {
        ...
        var colorWrapper = $('<div class="chroma-hash"></div>'),
        ...
        for(c in colors) {
          ...
          $input.data('chroma-hash-trigger', trigger);
